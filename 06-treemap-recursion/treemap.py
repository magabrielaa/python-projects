...
def recursive_helper(t_sorted, bounding_rec):
    '''
    Computes the rectangles for drawing a treemap of the provided tree.

    Inputs:
        t_sorted: list of Tree instances, sorted
        bounding_rec: a Rectangle object
    ...
    for k in range(len(t_sorted)):
        row_layout, leftover_rec = compute_row(bounding_rec,t_sorted[:k + 1], total_sum)
        a_ratios = []
        for rec, t in row_layout:
            aspect_ratio = max(rec.width, rec.height) / min(rec.width, rec.height)
            a_ratios.append(aspect_ratio)
        row_distortion = max(a_ratios)
        if not best_distortion or row_distortion <= best_distortion:
            best_layout = row_layout
            best_distortion = row_distortion
            best_k = k
            best_leftover = leftover_rec
        else:
            break
    
    rv = []
    for rec, t in best_layout:
        rec.label = t.key
        ...
        if t.num_children() == 0:
            ...
            # Recursive computation for treemap depth
            rv.extend(recursive_helper(sorted_children, rec))
       
    # Recursive computation for treemap breadth 
    rv.extend(recursive_helper(t_sorted[best_k + 1:], best_leftover))
    return rv
...
def compute_rectangles(t, bounding_rec_width=1.0, bounding_rec_height=1.0):
    ...
    compute_paths(t) 
    t_sorted = sorted_trees(t.children)
    bounding_rec = Rectangle((0.0,0.0), (bounding_rec_width, bounding_rec_height))
    
    return recursive_helper(t_sorted, bounding_rec)
...
def compute_internal_values(t):
    ...
    if t.num_children() == 0:
        ...
        for child in t.children:
            sum_values += compute_internal_values(child)
        t.value = sum_values
